...
import CustomCheckoutFormUpdated from "../components/CustomCheckoutFormUpdated";
...
export default function PaymentElementUpdated() {
  ...
  const options = {
    clientSecret,
    appearance,
    customerSessionClientSecret: `cuss_secret_QeVQCsG41dtDLeHY9SKQuyONaqwOrgMpGrK7MqvUuIYDeI6`
  };
    ...
    <div className="App">
      {clientSecret && (
        <Elements key = {clientSecret} options={options} stripe={stripePromise}>
          <CustomCheckoutFormUpdated subtotal={subtotal}/>
        </Elements>
      )}
    </div>
  )
